...
class Parser
{
    ...
    // added by Marian
    public function isProtectedPdf (string $filename): bool
    {
        $content = file_get_contents($filename);

        if (stristr($content, "/Encrypt"))
        {
            return true;
        }
        return false;
    ...
    // added by Marian
    public function isProtectedPdfTest (string $filename): bool
    {
        $content = file_get_contents($filename);

        echo nl2br("inside function\n\n");
        //list($xref, $data) = $this->rawDataParser->parseData($content);

        //echo nl2br($content);

        if (empty($content)) {
            echo nl2br ("Empty PDF data given.\n\n");
        }
        // find the pdf header starting position
        $trimpos = strpos($content, '%PDF-');

        if (false === $trimpos) {
            echo nl2br ("Invalid PDF data: missing %PDF header.\n\n");
        }

        echo nl2br($trimpos."\n\n");

        // get PDF content string
        $pdfData = substr($content, $trimpos);
        //echo nl2br($pdfData."\n\n");

        // get xref and trailer data
        $xref = $this->rawDataParser->getXrefData($pdfData);
        echo nl2br("**".$xref."**\n\n");
        ...
        echo nl2br("after parse data function\n\n");
        print_r($xref);
        echo nl2br("\n\n");
        print_r($objects);
        echo nl2br("\n\n");
